...
ol_output=tf.nn.softmax(ol_input)
# ol_output=tf.sigmoid(ol_input)

cross_entropy_val=tf.nn.softmax_cross_entropy_with_logits(logits=ol_output,labels=output_matrix)
loss=tf.reduce_mean(cross_entropy_val)

optimiser=tf.train.GradientDescentOptimizer(learning_rate=.05).minimize(loss)
